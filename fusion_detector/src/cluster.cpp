...
void Cluster::SetCloud_z(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_origin_cloud_ptr,
                        std_msgs::Header in_ros_header,
                        int in_id,
                        int in_r,
                        int in_g,
                        ...
                        bool in_estimate_pose)
{
  ...
  for (unsigned int pit = 0; pit < in_origin_cloud_ptr->points.size(); pit++)
  {
    // fill new colored cluster point by point
    pcl::PointXYZRGB p;
    p.x = in_origin_cloud_ptr->points[pit].x;
...
//printf("in_origin_cloud_ptr->points.size()= %d \n",in_origin_cloud_ptr->points.size());
  // pose estimation
  double rz = 0;
